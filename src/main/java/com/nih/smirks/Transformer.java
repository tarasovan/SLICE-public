...
public class Transformer {
	...
	*/
	...
	private IAtomContainer applyTransform(IReaction reactionSmirks, IAtomContainer aggregateProducts) {
		...
		//System.out.println("productRecords " + productRecords);
		...
		for (IBond bond : productsSmirks.bonds()) {
            ...
            if (addBond == true) {
                ...
                //bond made
                    ...
                    makeBond(aggregateProducts, configBegin, configEnd, qb);
					//System.out.println("NEW BOND "+ begin.getProperty(CDKConstants.ATOM_ATOM_MAPPING) + " "+end.getProperty(CDKConstants.ATOM_ATOM_MAPPING));
        ...
        for (IAtom at : aggregateProducts.atoms()) {
			...
			//remove extra hydrogens
			//if (at.isAromatic()) System.out.println(at.getIndex() + at.getSymbol() +" "+ ElementCalculation.calculateValence(at.getSymbol())+ " "+ bondSum(aggregateProducts, at) + " " + at.getImplicitHydrogenCount() );
		...
		/*for (IBond bond : productsSmirks.bonds()) {
			...
			QueryAtom end = (QueryAtom) bond.getEnd();
			int aam = begin.getProperty(CDKConstants.ATOM_ATOM_MAPPING);
			int bam = end.getProperty(CDKConstants.ATOM_ATOM_MAPPING);
			IAtom configBegin = getAtomByAam(aggregateProducts, aam);
			IAtom configEnd = getAtomByAam(aggregateProducts, bam);
			while(ElementCalculation.calculateValence(configBegin.getSymbol()) < bondSum(aggregateProducts, configBegin) + configBegin.getImplicitHydrogenCount() ){
				//System.out.println(configBegin.getSymbol() +" "+ bondSum(aggregateProducts, configBegin) + configBegin.getImplicitHydrogenCount() );
				for(IBond b : aggregateProducts.bonds()){
					if (b.contains(configBegin) && !b.contains(configEnd)) {
			...
			while(ElementCalculation.calculateValence(configEnd.getSymbol()) < bondSum(aggregateProducts, configEnd) + configEnd.getImplicitHydrogenCount() ){
				//System.out.println(configEnd.getSymbol() + " "+bondSum(aggregateProducts, configEnd) + configEnd.getImplicitHydrogenCount());
				for(IBond b : aggregateProducts.bonds()){
					if (b.contains(configEnd) && !b.contains(configBegin)) {
						aggregateProducts.removeBond(b);
						break;
					}
				}
	...
	private int bondSum(IAtomContainer mol, IAtom at){
		int sum = 0;
		//for aromatic atoms
		boolean double_bonds= false ,double_aromatic = false;
		for(IBond bond : mol.bonds()){
			...
			if(bond.contains(at)){
				...
				if(bond.getOrder() == DOUBLE && bond.isAromatic()){
					double_aromatic = true;
				}
			}
		}
		if(double_bonds){
	...
	private void configBond(QueryBond source, QueryBond target, IBond toModify) {
		...
		for (Entry<Type, List<Integer>> entry : exprs2.entrySet()) {
			...
			if(exprs2.containsKey(type) && type.equals(Expr.Type.IS_AROMATIC) ){
				//toModify.setOrder(IBond.Order.DOUBLE);
				toModify.setOrder(SINGLE);
		...
		//System.out.println("BOND ORDER end : "+toModify.getOrder() + " "+toModify.isAromatic());
	}
	...
	public String makeSmiles(IReaction reaction, boolean aam) throws CDKException {
		...
		//System.out.println("REACTION:"+ reaction.toString());
