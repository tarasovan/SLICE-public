...
public class SliceFacade extends SliceBaseVisitor<Value> {
	...
	public Value visitForeach_stmt(SliceParser.Foreach_stmtContext ctx) {
		...
		else if(loop.loop_predicate().set_current_ring_no_size() != null){
			Value v = visit(loop.loop_predicate().set_current_ring_no_size());
			if(v.isList()){
				list.addAll(v.asObjectList());
			}
			//SliceParser.Set_current_ringContext ringcontext = loop.loop_predicate().set_current_ring();
	...
	public Value visitTerm(SliceParser.TermContext ctx) {
		...
		if (ctx.term_block() != null) {
			...
			if (ctx.term_block().relation() == null) {
				...
				if (ctx.term_block().relation() != null) {
					//System.out.println("ctx.term_block().relation() "+ctx.term_block().relation().getText());
			...
			// comparison between numerical value
			//System.out.println("ctx.relation() "+ ctx.relation());
		...
		for (int i = 0; i < size ; i++) {
			...
			if (isItAnd){
				...
				if (andSubj.length == 1 && andSubj[0].isDouble()){
					...
					res[0] = andSubj[0];
					return new Value(andSubj[0].asDouble());
				}
				else {
	...
	public Value visitAnd_subject(SliceParser.And_subjectContext ctx) {
		...
		for (int i = 0; i < subjects.size(); i++) {
			result[i] = visitSubject(subjects.get(i));
		}
		...
		/*if(isItAnd == false)
		{
			System.out.println("value res :"+ result[0]);
		}*/
		return new Value(result);
		/*
	...
	public Value visitOr_Predicate(SliceParser.Or_predicateContext ctx) {
		//System.out.println("visitOr_Predicate" +ctx.getText());
	...
	public Value visitSubjects(SliceParser.SubjectsContext ctx){
		return visit(ctx.or_subject());
	}
	public Value visitSubject(SliceParser.SubjectContext ctx){
		if (ctx.subject_appendages()!= null) {
			return visit( ctx.subject_appendages() );
		}
		if (ctx.subject_atom()!= null) {
			return visit( ctx.subject_atom() );
		}
		if (ctx.subject_bond()!= null) {
			return visit( ctx.subject_bond());
		}
		if (ctx.subject_molecule()!= null) {
			return visit( ctx.subject_molecule());
		}
		if (ctx.subject_ring()!= null) {
			return visit( ctx.subject_ring());
		}
		if (ctx.subject_atom_locs()!= null) {
			return visit( ctx.subject_atom_locs() );
		}
		if (ctx.subject_or_predicate_atom_type()!= null) {
			return visit( ctx.subject_or_predicate_atom_type() );
		}
	return Value.VOID;
	}

public Value visitAtom1(SliceParser.Atom1Context ctx){
		if(ctx.subjects() != null){
			return visit( ctx.subjects() );
		}

		if(ctx.variable_name()!= null){
			return visit( ctx.variable_name() );
		}
		return Value.VOID;
}
	...
	public Value visitSubject_molecule(SliceParser.Subject_moleculeContext ctx) {
		//System.out.println("Subject molecule :"+ ctx.getText());
		if(ctx.NUMBERATOMS() != null){
			return new Value(Double.valueOf(ac.getAtomCount()));
		}
		else {
			List<IChemObject> res = new ArrayList<IChemObject>();
			res.add(ac);
	...
	public Value visitPredicate_atom_property(SliceParser.Predicate_atom_propertyContext ctx) {
		...
		//System.out.println("predicate atom prop: "+ toReturn);
		return new Value(toReturn);
	}
	...
	public Value visitPredicate_bond_property(SliceParser.Predicate_bond_propertyContext ctx) {
		...
		//System.out.println("predicate_bond prop: "+toReturn);
		return new Value(toReturn);
	}
	...
	public Value visitPredicate_ring(SliceParser.Predicate_ringContext ctx) {
		//System.out.println("visitPredicate_ring "+ ctx.getText());
		SliceSet cs = new SliceSet();
		IRingSet allRings = null;
		try {
			allRings = cs.getAllRing(ac);
		...
		for (IAtomContainer ac2 : allRings.atomContainers()) {
			...
			if (ctx.ring_size() != null) {
				...
				if (ctx.OR_SMALLER() != null) {
					...
					if (ac2.getAtomCount() >= target && ac2.getAtomCount() <= Integer.parseInt(ctx.INT(1).getText()) ) {
						keepRing = true;
					} else {
						keepRing = false;
					}
		...
		//System.out.println("size of set : "+ newSet.getAtomContainerCount());
				...
				for (IChemObject obj : objects) {
					if (ctx.ring_prefix_prop() != null) {
						...
						if (obj instanceof IAtom) {
							...
							int res = 1;
							try {
								//System.out.println("Atom : "+ ((IAtom) obj).getSymbol()+((IAtom) obj).getIndex());
								res = sa.isInCurrentRing((IAtom) obj, newSet) ? 1 : -1;
								// no need to check with relationType as rings have already been filtered using
								// it.
								// The only thing to check is if the current atom is part of the filtered ring
		...
		//System.out.println("predicatering : "+toReturn);
		return new Value(toReturn);
	}
	...
	public Value visitSet_current_ring(SliceParser.Set_current_ringContext ctx) {
		...
		ExtractSubjectFromAtomContainer sf = new ExtractSubjectFromAtomContainer();
		List<IChemObject> refs = new ArrayList<IChemObject>();
		//System.out.println("size "+ ctx.INT().size());
		if (ctx.INT().size() == 2) {
						...
						if(ring.getAtomCount() == size_limit) {
							for (IAtom a : ring.atoms()) {
								...
								if (ctx.ring_prefix_prop() != null) {
									...
									if (ctx.ring_prefix_prop().HETEROCYCLIC() != null ) {
										if (!a.getSymbol().equals("C")) {
											target = ring;
										} else {
											target = null;
	...
	public Value visitSet_current_ring_no_size(SliceParser.Set_current_ring_no_sizeContext ctx) {
		//System.out.println("CTX:" +ctx.getText());
		List<IChemObject> res = new ArrayList<IChemObject>();
		SliceSet cs = new SliceSet();
