...
value : (variable_name|alpha_num|numeric_num|bool|subjects|set_current_ring_no_size|set_current_ring);
...
subject_molecule : MOLECULE | NUMBERATOMS;
...
loop_predicate : MOLECULE | loop_predicate_ring | variable_name | set_current_ring_no_size| set_current_ring | (CURRENT RING) | (INTERFERING GROUP) | (PARTICIPATING GROUP);
...
set_current_ring_no_size : (THE|ring_prefix_prop) (ALIPHATIC|AROMATIC|(ALIPHATIC OR AROMATIC)) RING CONTAINED (ATOM INT | variable_name);
...
NUMBERATOMS : 'the number of atoms in molecule';
