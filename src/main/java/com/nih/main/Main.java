...
public class Main {
	...
	private static void generateMolecules(CommandLine commands, Options options, String format) throws CDKException, IOException, CloneNotSupportedException {
		...
		int size_max, size_limit_per_file;
		...
		if (commands.hasOption('z')){
			size_limit_per_file = Integer.parseInt(commands.getOptionValue('z'));
		}
		else {
			size_limit_per_file = 0;
		}
		if (commands.hasOption('p')){
			size_max = Integer.parseInt(commands.getOptionValue('p'));
		}
		else {
			size_max = 0;
		}
		...
		core.setSize_max(size_max);
		core.setSize_limit_per_file(size_limit_per_file);
		core.exec();
	}
