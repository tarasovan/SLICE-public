...
public class ChemicalFormatParser {
	...
	public String formatDetector(String fileName) throws IOException {
		...
		if (file.isFile()) {
			...
			if (element.isJsonObject()) {
				JsonObject sliceCode = element.getAsJsonObject();
				if (sliceCode.get("ChemEditor") != null)
					return "sliceJSON";
			}
			...
			while ((str = br.readLine()) != null) {
				if (!str.isEmpty()) {
					if (str.contains("<transforms>")) {
						format = "sliceXML";
						break;
					}
					else if (str.contains("$RXN")) {
						format = "RXN_FILE";
						break;
					}
	...
	public Map<Integer, Slice> parseJSlice(String fileName) throws CDKException, IOException {
		JsonElement element = JsonParser.parseReader(new FileReader(fileName));
		if (element.isJsonObject()) {
			JsonObject sliceCode = element.getAsJsonObject();
			if (sliceCode.get("ChemEditor") != null) {
				SliceFileParser parser = new SliceFileParser();
				return parser.parse(new ByteArrayInputStream(sliceCode.get("ChemEditor").getAsString().getBytes()));
			}
			else 
				return null;
		}
	...
	public Map<Integer, Slice> parseSliceXML(String fileName) throws CDKException, IOException {
		SliceFileParser parser = new SliceFileParser();
...
//				LOGGER.error(INFO, "Annotating Reaction " + "smiles");
	 ...
	 */
	/*
	 * public IReactionSet parseReactionCode(String fileName) throws CDKException,
	 * IOException, CloneNotSupportedException { List<String> reactionCodes = new
	 * ArrayList<String>(); if (new File(fileName).isFile()) { BufferedReader br =
	 * new BufferedReader(new FileReader(fileName)); String line; String
	 * reactionCode = ""; while ((line = br.readLine()) != null) { if
	 * (line.contains("\\:") && line.charAt(0) == '0') { if (reactionCode.length() >
	 * 0) reactionCodes.add(reactionCode); reactionCode = line; } else if
	 * (line.contains("\\:") && line.charAt(0) != '0') { reactionCode += line; } }
	 * br.close(); } DecodeReactionCode decoder = new DecodeReactionCode();
	 ...
	 */
	/*
	 * public IReactionSet parseReactionCode(String fileName, String format) throws
	 * CDKException, IOException, CloneNotSupportedException { List<String>
	 * reactionCodes = new ArrayList<String>();
	 * 
	 * if (format.equals("REACTIONCODE_FILE")) { BufferedReader br = new
	 * BufferedReader(new FileReader(fileName)); String line; String reactionCode =
	 * ""; while ((line = br.readLine()) != null) { if (line.contains(":") &&
	 * line.charAt(0) == '0') { if (reactionCode.length() > 0)
	 * reactionCodes.add(reactionCode); reactionCode = line; } else if
	 * (line.contains(":") && line.charAt(0) != '0') { reactionCode += line; } } if
	 * (reactionCode.length() > 0) reactionCodes.add(reactionCode); br.close(); }
	 * else if (format.equals("REACTIONCODE_TEXT")) { reactionCodes.add(fileName); }
	 * else if (format.contains("REACTIONCODE_CSV_")) { CSVReader reader = null; try
	 * { int column = Integer.parseInt(format.split("_")[3]); char separator = ',';
	 * 
	 * if (format.contains("SPACE")) { separator = ' '; } else if
	 * (format.contains("TAB")) { separator = '\t'; } final CSVParser parser = new
	 * CSVParserBuilder().withSeparator(separator).build(); if
	 * (format.contains("REACTIONCODE_CSV_WITHHEADER_")) { reader = new
	 * CSVReaderBuilder(new
	 * FileReader(fileName)).withSkipLines(1).withCSVParser(parser).build(); } else
	 * { reader = new CSVReaderBuilder(new
	 * FileReader(fileName)).withCSVParser(parser).build(); } reader = new
	 * CSVReader(new FileReader(fileName)); String[] line; while ((line =
	 * reader.readNext()) != null) { reactionCodes.add(line[column]); }
	 * reader.close(); } catch (IOException e) { e.printStackTrace(); } }
	 * 
	 * DecodeReactionCode decoder = new DecodeReactionCode(); IReactionSet
	 * reactionsSet = decoder.makeReactions(reactionCodes);
	 * 
	 * return reactionsSet; }
	 */
	 ...
	 */
	/*
	 * public List<String> getReactionCode(String fileName, String format, boolean
	 * split) throws CDKException, IOException, CloneNotSupportedException {
	 * List<String> reactionCodes = new ArrayList<String>();
	 * 
	 * if (format.equals("REACTIONCODE_FILE")) { int cpt = 0; reactionCount = 0;
	 * Path path = Paths.get(fileName); long lineCount = Files.lines(path).count();
	 * FileWriter writer = null; File directory = new File(new
	 * File(fileName).getParent(), "tempRCFiles/"); if (lineCount > 10000) {
	 * files.clear(); if (!directory.exists()){ directory.mkdir(); } }
	 * BufferedReader br = new BufferedReader(new FileReader(fileName)); String
	 * line; String reactionCode = ""; while ((line = br.readLine()) != null) { if
	 * (line.contains(":") && line.charAt(0) == '0') { if (reactionCode.length() >
	 * 0) { if (split) { if (reactionCount >= 10000) { if (cpt == 10000) { if
	 * (writer != null) writer.close(); File temp = new File(directory.toString() +
	 * "/chunk_" + files.size() + ".txt"); files.add(temp); writer = new
	 * FileWriter(temp); writer.write(reactionCode + System.lineSeparator()); cpt =
	 * 0; System.out.println(ColouredSystemOutPrintln.ANSI_GREEN + reactionCount +
	 * " reactions are counted and a temp file has been created" +
	 * ColouredSystemOutPrintln.ANSI_RESET); } else { writer.write(reactionCode +
	 * System.lineSeparator()); } } else if (reactionCount < 10000) {
	 * reactionCodes.add(reactionCode); } cpt++; } else {
	 * reactionCodes.add(reactionCode); } reactionCount++; } reactionCode = line; }
	 * else if (line.contains(":") && line.charAt(0) != '0') { reactionCode += line;
	 * } } if (reactionCode.length() > 0) { cpt++; reactionCount++; if (split) { if
	 * (reactionCount > 10000) { writer.write(reactionCode +
	 * System.lineSeparator()); if (writer != null) writer.close(); } else {
	 * reactionCodes.add(reactionCode); } } else { reactionCodes.add(reactionCode);
	 * } } br.close(); if (writer != null) writer.close(); } else if
	 * (format.equals("REACTIONCODE_TEXT")) { reactionCodes.add(fileName); } else if
	 * (format.contains("REACTIONCODE_CSV_")) { CSVReader reader = null; try { int
	 * column = Integer.parseInt(format.split("_")[3]); char separator = ',';
	 * 
	 * if (format.contains("SPACE")) { separator = ' '; } else if
	 * (format.contains("TAB")) { separator = '\t'; } final CSVParser parser = new
	 * CSVParserBuilder().withSeparator(separator).build(); if
