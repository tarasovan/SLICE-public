...
* Rules
*/
//TODO Fix compparison more than == GT and ess than
//TODO including hydrogen
...
select_chemical_object : select_chemical_object_by_id | select_chemical_object_by_range | chemical_object;
select_chemical_object_by_id : article? chemical_object INT ;
//select_chemical_object_by_loc : article? chemical_object atom_loc ;
select_chemical_object_by_range : article? chemical_object range ;

select_chemical_objects : select_chemical_objects_by_locs | select_chemical_objects_by_type | select_chemical_object_by_set;
select_chemical_objects_by_locs : (article | alpha_num | INT | ANY | ALL)? (atomtype|centre_stmt)? chemical_object? WITHIN? atom_loc preposition? atomtype? ((chemical_object INT?)|var)? ;
select_chemical_objects_by_type : (article | alpha_num | INT | ANY | ALL)? atomtype chemical_object? ;
select_chemical_object_by_set : article? (chemical_object preposition)? properties_stmt ;
select_chemical_objects_by_appendages : appendages_start_stmt (AND appendages_suite_stmt)* ;
appendages_start_stmt : article? APPENDAGE FROM ((select_chemical_object) | (atom_loc preposition? (chemical_object|var)?) | var) ANYWHERE? where_stmt? preposition article? ((select_chemical_object) | (atom_loc preposition? (chemical_object|var)?) | var)  where_stmt? ;
appendages_suite_stmt : article? APPENDAGE? FROM ((select_chemical_object) | (atom_loc preposition? (chemical_object|var)?) | var) ANYWHERE? where_stmt? preposition article? ((select_chemical_object) | (atom_loc preposition? (chemical_object|var)?) | var) where_stmt? ;

select_chemical_groups : select_chemical_groups_by_type | select_chemical_groups_by_locs;
select_chemical_groups_by_locs : (article | alpha_num | INT | ANY | ALL)? (smarts | functional_group | other_group | group_prefix) GROUP? WITHIN? atom_loc preposition? atomtype? ((chemical_object INT?)|var)? ;
select_chemical_groups_by_type : (article | alpha_num | INT | ANY | ALL)? (smarts | functional_group | other_group | group_prefix) GROUP?;

properties_stmt : atom_prop_stmt | bond_prop_stmt | group_prop_stmt | ring_prop_stmt | centre_stmt | charge_stmt;
//TODO ducplicate with type remove => ((alpha_num | INT | ANY)? atomtype chemical_object?) ;
atom_prop_stmt : ((alpha_num | INT | article)? atom_prefix chemical_object? atom_loc?) 
    | ((alpha_num | INT | ANY)? atomtype chemical_object?)
    | atom_suffix ;
bond_prop_stmt : (alpha_num |INT | ANY | article)?  bond_prefix chemical_object? ;
group_prop_stmt : (alpha_num | INT | ANY | article)? (smarts | functional_group | other_group | group_prefix) ;
ring_prop_stmt : (ONE|article)? ring_prefix_prop? AROMATIC? RING ring_size? (INT| alpha_num)? comp_op? (INT| alpha_num)? (CONTAINED complex_poststmt)? ;

loop_stmt : loop_complex_prestmt var? (preposition article? (func_stmt|var|set))?
...
loop_select_chemical_objects_by_locs : (article | alpha_num | INT | ANY | ALL)? (atomtype| centre_stmt)? chemical_object? WITHIN? atom_loc preposition? atomtype? ((chemical_object INT?)|var)? ;
loop_select_chemical_objects_by_type : (article | alpha_num | INT | ANY | ALL)? atomtype chemical_object? ;
loop_select_chemical_objects_by_prop : (alpha_num | INT | article)? (atom_prefix|bond_prefix|((smarts|functional_group|other_group|group_prefix)GROUP?)) chemical_object? ;
loop_select_chemical_object_related_to : article? (bond_prop_stmt|atom_prop_stmt|((smarts|functional_group|other_group|group_prefix)GROUP?)) preposition chemical_object;

loop_select_chemical_groups : select_chemical_groups_by_type | select_chemical_groups_by_locs;
loop_select_chemical_groups_by_locs : (article | alpha_num | INT | ANY | ALL)? (smarts | functional_group | other_group | group_prefix) GROUP? (WITHIN|preposition)? atom_loc preposition? atomtype? ((chemical_object INT?)|var)? ;
loop_select_chemical_groups_by_type : (article | alpha_num | INT | ANY | ALL)? (smarts | functional_group | other_group | group_prefix) GROUP?;



//old way
//comparison : expr ((IS|HAS)? comp_op expr)* ;
//comp_op: LT|GT|GE|LE|NE|FEWER_THAN|FEWER|OR_LARGER|LARGER|OR_SMALLER|SMALLER|LESS_THAN|MORE_THAN|LESS_HINDERED|MORE_HINDERED|THROUGH|notcontain|notequal|equal|contain ;
//expr: or_expr ;
//or_expr : and_expr (OR and_expr)* ;
//and_expr: arith_expr (AND arith_expr)* ;
//arith_expr: term ((PLUS|MINUS) term)* ;
//term: alpha_num|numeric_num|bool|var|complex_prestmt ;
//var : NAME (numeric_num|INT)? ;
...
//or_expr : and_expr (OR and_expr)* ;
//and_expr: arith_expr (AND arith_expr)* ;
//arith_expr: term ((PLUS|MINUS|TIMES|DIV|MODULUS) term)* ;

//loop_stmt : prestmt (NOT? preposition term)? ANYWHERE? where_stmt? var? 
//    | var EQ prestmt (NOT? preposition term)? ANYWHERE? where_stmt? ;

//multi prestmt selection
//multiselect_stmt : exp ;
//exp: or_exp ;
//or_exp : and_exp (OR and_exp)* ;
//and_exp: select_stmt (AND select_stmt)* ; 
...
charge_stmt : (article|alpha_num|INT)? (MINUS_CHARGE | PLUS_CHARGE) ;

centre_stmt : article? (PRIMARY_CENTER | SECONDARY_CENTER | TERTIARY_CENTER | QUATERNARY_CENTER) ;
