...
public class SliceTester {
	...
	public static void checkProducts(String outputPath, String outputName,int file_id, String sliceFilepath, String lowerRating, String ghostMolecules, String inchikey) throws CloneNotSupportedException, IOException {

		//List<String> ghost_molecules= new ArrayList<>(); 
		//ghost_molecules = print_ghost_molecules(sliceFilepath);

		System.out.println("outfile file :" + outputPath + outputName + "_0_file_"+ String.format("%03d",file_id)+".csv"); // file with products
		//write sdf files of products
		//System.out.println("outputPath"+ outputPath+outputName);
		//System.out.println("outputName"+ outputName);\
		BufferedReader reader_id = new BufferedReader(new FileReader((outputPath + outputName).split("\\.")[0].concat("_0_file_"+ String.format("%03d",file_id)+".id")));
		Path filePath = Paths.get((outputPath + outputName).split("\\.")[0].concat("_0_file_"+ String.format("%03d",file_id)+".sdf"));
		...
		BufferedWriter outputsdf = new BufferedWriter(new FileWriter((outputPath + outputName).split("\\.")[0].concat("_0_file_"+ String.format("%03d",file_id)+".sdf")));
		filePath = Paths.get((outputPath + outputName).split("\\.")[0].concat("_0_file_"+ String.format("%03d",file_id)+"_identifier.smi"));
		...
		BufferedWriter output_id = new BufferedWriter(new FileWriter((outputPath + outputName).split("\\.")[0].concat("_0_file_"+ String.format("%03d",file_id)+"_identifier.smi")));
		output_id.write("SMILES\tSTRID\n");
		...
		try {
			...
			if (ghostMolecules.equals(activated)) {
				filePath = Paths.get(outputPath + outputName + "_file_" + String.format("%03d", file_id) + "_no_ghost.csv");
				...
				outputFile = new BufferedWriter(new FileWriter(outputPath + outputName + "_file_" + String.format("%03d", file_id) + "_no_ghost.csv"));
			}
			else {
				filePath = Paths.get(outputPath + outputName + "_file_" + String.format("%03d", file_id) + "_with_ghost.csv");
				...
				outputFile = new BufferedWriter(new FileWriter(outputPath + outputName + "_file_" + String.format("%03d", file_id) + "_with_ghost.csv"));
			}//System.out.println("output file created");
			reader = new BufferedReader(new FileReader(outputPath + outputName + "_0_file_"+ String.format("%03d",file_id)+".csv"));
			//outputFile = new BufferedWriter(new FileWriter(outputPath+outputName+".csv")); 
			outputFile2 = new BufferedWriter(new FileWriter(outputPath + outputName + "_0_file_"+ String.format("%03d",file_id)+"_delete.csv"));
			//String header = reader.readLine();
			String line = reader.readLine();

			String id_product = reader_id.readLine();
			String header = "Products" +"\t" + "Product_ID" + "\t" + "SMIRKS" + "\t" + "Score";
					...
					try {
						...
						for (Slice sc : sliceTransforms.values()) {
							for (SliceReaction scr : sc.getReactions()) {
								int nb_product = 0;
								for (SliceMolecule mol : scr.getProducts()) {
									//System.out.println(mol.smarts + " ----"+ product);
									if (!list_products.contains(mol.smarts)) {
										...
										//System.out.println("MATCHING "+ ptrn.matches(final_t) +" "+dre);
				...
				if (!final_line.isEmpty()) {
					...
					if (!inchikey.equals(activated)) {
						...
						try {
							...
							output_id.write(smilesGenerator.create(product));
							output_id.write("\t ");
							output_id.write(id_product);
							output_id.write("\n");
					...
					if (ghostMolecules.equals(activated)) {
						if (inchikey.equals(activated)) {
							...
							outputFile.write(final_line + "\t" + line.split("\t")[1] + "\t" + line.split("\t")[2] + "\t" + (new_rating + Integer.parseInt(line.split("\t")[3])));
						...
						if (inchikey.equals(activated)) {
							...
							if (final_line.split("\\.").length == products.length) {
								IAtomContainer prod = sp.parseSmiles(line.split("\t")[0]);
								AtomContainerManipulator.convertImplicitToExplicitHydrogens(prod);
								SmilesGenerator gen = new SmilesGenerator(SmiFlavor.Default);
								String final_prod = gen.create(prod);
								outputFile.write(final_prod + "\t" + line.split("\t")[1] + "\t" + line.split("\t")[2] + "\t" + (new_rating + Integer.parseInt(line.split("\t")[3])));
								outputFile.newLine();
							} else {
								String cleanProd = removeAromacity(line.split("\t")[0]);
								IAtomContainer prod = sp.parseSmiles(cleanProd);
								AtomContainerManipulator.convertImplicitToExplicitHydrogens(prod);
								SmilesGenerator gen = new SmilesGenerator(SmiFlavor.Default);
								String final_prod = gen.create(prod);
			...
			output_id.close();
			//DEBUG DELETE FILES
			(new File(outputPath + outputName + "_0_file_"+ String.format("%03d",file_id)+".csv")).delete();
			(new File(outputPath + outputName + "_0_file_"+ String.format("%03d",file_id)+"_delete.csv")).delete();
			(new File((outputPath + outputName).split("\\.")[0].concat("_0_file_"+ String.format("%03d",file_id)+".id"))).delete();
			if (inchikey.equals(activated)) {
				(new File((outputPath + outputName).split("\\.")[0].concat("_0_file_"+ String.format("%03d",file_id)+".sdf"))).delete();
			}
		} catch (IOException e) {
			e.printStackTrace();
		} catch (InvalidSmilesException e) {
							...
							try {
								...
								if (ptrn.matches(ac1)) {
									...
									for (Map<IAtom, IAtom> map : mapping.toAtomMap()) {
										...
										Map<Integer, Integer> aam = convertAAMapToIndexAtomConstantMap(ac2, map);
										configAtomContainer(ac2, aam, map.values());
										//System.out.println(" mol.getLogic()  =>" + mol.getLogic());
										if (mol.getLogic() != null) {
											ParseTree tree = getParseTreeFromString(mol.getLogic());
											SliceFacade scf = new SliceFacade(ac2);
											if (carbonCentered != null)
											...
											IAtomContainer newAc = ac2.clone();
											newAc.setProperty("compatibleBB", mol.getId());
											newAc.setProperty("rating", 0);
											newAc.setProperty("sliceScore", 0);
											mappingToMolecule.put(maapedAtomIndex, newAc);
										}
