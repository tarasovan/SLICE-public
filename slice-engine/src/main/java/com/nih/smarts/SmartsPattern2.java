 ...
 * @author John May
 */
public final class SmartsPattern2 extends Pattern {
     ...
     */
    private SmartsPattern2(final String smarts, IChemObjectBuilder builder) {
        this.query = new QueryAtomContainer(builder);
        if (!Smarts2.parse(query, smarts))
            throw new IllegalArgumentException("Could not parse SMARTS: " +
                                               smarts + "\n" +
                                               Smarts2.getLastErrorMesg() + "\n" +
                                               Smarts2.getLastErrorLocation());
        this.pattern = Pattern.findSubstructure(query);
    }

    //MODIFICATION add constructor with flavor
    /**
     * Internal constructor.
     * @param smarts
     * @param flavor
     * @param builder
     */
    private SmartsPattern2(final String smarts, int flavor, IChemObjectBuilder builder) {
        this.query = new QueryAtomContainer(builder);
        if (!Smarts2.parse(query, smarts, flavor))
            throw new IllegalArgumentException("Could not parse SMARTS: " +
                                               smarts + "\n" +
                                               Smarts2.getLastErrorMesg() + "\n" +
                                               Smarts2.getLastErrorLocation());
        /*for (IBond bond : query.bonds()) {
            System.out.println(((QueryAtom) bond.getBegin()).getExpression()+ " " + bond.getOrder() + " " + ((QueryAtom) bond.getEnd()).getExpression() + " " +bond.isAromatic());
            QueryBond b = (QueryBond) bond;
            System.out.println(" "+b.getExpression());
        }*/
        /*for (IAtom a : query.atoms()) {
        	QueryAtom at = (QueryAtom) a;
        	System.out.println(at.getExpression());
        }*/
        this.pattern = Pattern.findSubstructure(query);
    }
    //END MODIFICATION
    ...
    public static void prepare(IAtomContainer target) {
        ...
        try {
            ...
            LoggingToolFactory.createLoggingTool(SmartsPattern2.class).error(e);
        }
    }
     ...
     */
    public SmartsPattern2 setPrepare(boolean doPrep) {
        this.doPrep = doPrep;
        return this;
    }
     ...
     */
    public static SmartsPattern2 create(String smarts, IChemObjectBuilder builder) {
        return new SmartsPattern2(smarts, builder);
    }
     ...
     */
    public static SmartsPattern2 create(String smarts) {
        return new SmartsPattern2(smarts, null);
    }
    
    //MODIFICATION add a constructor with flavor
    /**
     * Default SMARTS pattern constructor, passes in a null chem object builder.
     * 
     * @param smarts
     ...
     * @return
     */
    public static SmartsPattern2 create(String smarts, int flavor) {
        return new SmartsPattern2(smarts, flavor, null);
    }
    //END
}
