...
public final class Smarts2 {
    ...
    private static final class Parser {
        ...
        //add set barom to let 
        private Set<IBond>              barom = new HashSet<>();
        ...
        IBond addBond(IAtom atom, IBond bond) {
            if (atom.equals(bond.getBegin())) {
                ...
                mol.addBond(bond);
                //System.out.println(bond.getBegin().getSymbol() + " " + bond.getOrder() + " " + bond.getEnd().getSymbol() + " " +bond.isAromatic());
        ...
        boolean parseExplicitHydrogen(IAtom atom, Expr dest) {
            ...
            Expr hExpr = isotope < 0 ?
                         ...
                         new Expr(Expr.Type.AND,
                                  ...
                                  new Expr(Expr.Type.ELEMENT, 1));
            if (isotope > 0) atom.setMassNumber(isotope);   
        ...
        private boolean parseRange(Expr expr) {
          ...
          //MODIFICATION parse range when hi is not defined
                    ...
                    case 'A':
                        switch (next()) {
                            ...
                            case 's': // As=Arsenic
                                ...
                                break;
                            case 't': // At=Astatine
                            	atom.setSymbol(Elements.ofNumber(85).symbol());
                        ...
                        switch (next()) {
                            ...
                            default:  // Z=None
                                ...
                                if (isFlavor(FLAVOR_DAYLIGHT)) {
                                 ...
                                 // MODIFICATION to parse range for 
                                } else if (isFlavor(FLAVOR_CACTVS)) {
                                    if (num < 0) {
                    ...
                    case 'b':
                        switch (next()) {
                            default:  // b=Boron (aromatic)
                                ...
                                expr.or(new Expr(ELEMENT, 5));
                                break;
                        }
                        break;
                    case 'c':
                        expr = new Expr(AROMATIC_ELEMENT, 6);
                        expr.or(new Expr(ELEMENT, 6));
                        break;
                    //MODIFICATION add e case
                    case 'e':
            ...
            while (true) {
                ...
                // MODIFICATION expr for single and double bond The commented version is the original one
                    ...
                    case '=':
                    	...
                    	expr = new Expr(DOUBLE_OR_AROMATIC);
                        break;
                    //END MODIFICATION
                    case '#':
                    	//MODIF ADD electron in bond count
                    	 int num = nextUnsignedInt();
                         if (num < 0) {
                             if (isFlavor(FLAVOR_CACTVS)) {
                                 switch (next()) {
                                     case 'E':
                                         num = nextUnsignedInt();
                                         if (num < 0) {
                                             expr = new Expr(Expr.Type.ELECTRON_COUNT, 1);
                                             // CACTVS style ranges H{0-2}
                                             if (peek() == '{' && !parseRange(expr))
                                                 return false;
                                         }
                                         else
                                             expr = new Expr(Expr.Type.ELECTRON_COUNT, num);
                                     default:
                                         return false;
                                 }
        ...
        private boolean parseAtomExpr() {
            atom = new QueryAtom(mol.getBuilder());
        ...
        boolean finish() {
            ...
            if (!bstereo.isEmpty()) {
                ...
                for (IBond bond : bstereo) {
                   ...
                   //Modification add both expr.or
                    if (expr == null) {
                    	expr = new Expr(SINGLE_OR_AROMATIC);
                    	expr.or(new Expr(DOUBLE_OR_AROMATIC));
                    }
                    else {
            ...
            //MODIFICATION to make CDK more permissive for bond matching
            for (IBond bond : barom) {
            	if (bond.getBegin().isAromatic() && bond.getEnd().isAromatic()) {
            		QueryBond bond2 = (QueryBond) bond;
            		if (bond2.getExpression().type().equals(ALIPHATIC_ORDER)) {
            			int value = bond2.getExpression().value();
            			if (value == 2)
            				((QueryBond) BondRef.deref(bond)).setExpression(new Expr(DOUBLE_OR_AROMATIC));
                		else
                			((QueryBond) BondRef.deref(bond)).setExpression(new Expr(SINGLE_OR_AROMATIC));
            		}
            	}
    ...
    private static boolean generateBond(StringBuilder sb, Expr expr) {
        switch (expr.type()) {
            ...
            case SINGLE_OR_DOUBLE:
                ...
                break;
            case ORDER:
                LoggingToolFactory.createLoggingTool(Smarts2.class)
        ...
        private void generateAtom(StringBuilder sb,
            ...
            switch (expr.type()) {
                ...
                case NOT:
                    ...
                    break;
                case RECURSIVE:
                    sb.append("$(").append(Smarts2.generate(expr.subquery())).append(")");
                    break;
                case STEREOCHEMISTRY:
