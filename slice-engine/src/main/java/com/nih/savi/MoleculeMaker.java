...
public class MoleculeMaker {
	...
	private long nb_reactions = 0;
	private int size_limit_per_file;
	private int size_max;
	private String writer_file_name;
	private String writer_id_prod_file_name;
	private long count_in_file;
	...
	public void makeMolecules(Map<String, List<IAtomContainer>> map) {
		...
		System.out.println("size limit per file "+ size_limit_per_file);
		System.out.println("max products "+ size_max);

		if (size_limit_per_file ==0)
			size_limit_per_file = Integer.MAX_VALUE;
		if(size_max == 0)
			size_max = Integer.MAX_VALUE;
		...
		try {
			...
			outputWriter.newLine();
			for (int i : chunks.keySet()) {
				System.out.println(outputFile.split("\\.")[0]+".csv");
				BufferedReader reader = new BufferedReader(new FileReader(outputFile.split("\\.")[0]+".csv"));
				String line;
				...
				reader.close();
				//delete temp file
				new File(outputFile.split("\\.")[0]+".csv").delete();
			}
			outputWriter.close();
			...
			for (int i : chunks.keySet()) {
				...
				BufferedReader reader = new BufferedReader(new FileReader(outputFile.split("\\.")[0]+".id"));
				String line;
				while ((line = reader.readLine()) != null) {
					outputWriter.write(line);
					outputWriter.newLine();
				...
				reader.close();
				//delete temp file
				new File(outputFile.split("\\.")[0]+".id").delete();
			}
			outputWriter.close();*/
	...
	private boolean combine(int chunk, Map<Integer, List<IAtomContainer>> chunks, Collection<List<IAtomContainer>> otherReactants) {
		...
		writer_file_name = outputFile.split("\\.")[0]+"_file_001.csv";
		writer_id_prod_file_name = outputFile.split("\\.")[0]+"_file_001.id" ;

		System.out.println("file name "+ writer_file_name);
		count_in_file = 0;
		file_index = 1;

		start.addAll(otherReactants);
		GenerateAllCombinationsFromMultipleLists(start, 0, new ArrayList<IAtomContainer>(),chunk);
		System.out.println("combine DONE for chunk "+chunk);

		return true;
	}

	private void rotateFiles(int chunk) {
		if(count_in_file >=size_limit_per_file){
			file_index++;
			writer_file_name = String.format(outputFile.split("\\.")[0]+"_file_%03d.csv", file_index);
			writer_id_prod_file_name = String.format(outputFile.split("\\.")[0]+"_file_%03d.id", file_index);
			System.out.println("switching files ");
			System.out.println(writer_file_name);
			System.out.println(writer_id_prod_file_name);
	...
	private void GenerateAllCombinationsFromMultipleLists(List<List<IAtomContainer>> Lists, int depth, List<IAtomContainer> current,
		...
		if(depth == Lists.size()) {
			...
			if (current.size() == depth ) {
				...
				for (IReaction reaction : reactions) {
					...
					writeReactionToCSV(reaction, score);
					writeReaction_id_product(reaction);
					//writeReactionToSDF(reaction, writer_sdf, score);
					count_in_file++;
					if (nb_reactions >= size_max)
					{
						stop = true;
						return;
					}
		...
		for(int i = 0; i < Lists.get(depth).size(); i++) {
			...
			temp.add(ac);
			GenerateAllCombinationsFromMultipleLists(Lists, depth + 1, temp,chunk);
		}
	}
	...
	private void writeReactionToCSV(IReaction reaction, int score) {
		...
		//System.out.println("writeReactionToCSV : " +writer_file_name);
		BufferedWriter writer = null;
		try {
			writer = new BufferedWriter(new FileWriter(writer_file_name, true));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		line += split[split.length-1] + "\t" + id_product + "\t" + smi;
		line += "\t" + score;
	...
	private void writeReaction_id_product(IReaction reaction) {

		BufferedWriter writer_id_product = null;
		try {
			writer_id_product = new BufferedWriter(new FileWriter(writer_id_prod_file_name, true));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int getSize_limit_per_file() {return size_limit_per_file;}

	public void setSize_limit_per_file(int size_limit_per_file) { this.size_limit_per_file = size_limit_per_file;}

	public int getSize_max() { return size_max;}

	public void setSize_max(int size_max) { this.size_max = size_max;}

	public long getNb_reactions() {
		return nb_reactions;
	}

	public void setNb_reactions(long nb_reactions) {
		this.nb_reactions = nb_reactions;
	}
