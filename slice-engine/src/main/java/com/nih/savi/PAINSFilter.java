...
public class PAINSFilter {
	
	private Object[][] painsA;
	private Object[][] painsB;
	private Object[][] painsC;
	 ...
	 * @param ac
	 */
	public void calculate(IAtomContainer ac) {
		// clone IAtomContainer because matchAll cannot be used for matching multiple
		// SMARTS against the same container.
		for (Object[] filter : painsA) {
			String smarts = (String) filter[1];
			Pattern ptrn = SmartsPattern.create(smarts);
			try {
				int score = ptrn.matchAll(ac.clone()).countUnique();
				filter[2] = score;
				ac.setProperty((String) filter[0], score);
			} catch (CloneNotSupportedException e) {
				// TODO Auto-generated catch block
				filter[3] = "matching error";
				ac.setProperty((String) filter[0], -1);
				e.printStackTrace();
			}
		}
		for (Object[] filter : painsB) {
			String smarts = (String) filter[1];
			Pattern ptrn = SmartsPattern.create(smarts);
			try {
				int score = ptrn.matchAll(ac.clone()).countUnique();
	...
	/**
	 * Calculate only for those associated to true
	 * @param ac
	 * @param reactionPAINSFilter
	 */
	public String calculate(IReaction reaction, IAtomContainer ac, Map<String, Map<String, Boolean>> reactionPAINSFilter) {
		// clone IAtomContainer because matchAll cannot be used for matching multiple
		// SMARTS against the same container.
		String result = "";
		for (Object[] filter : painsA) {
			Map<String, Boolean> painsPreprocessing = reactionPAINSFilter.get("painsA");
			String nameFilter = (String) filter[0];
			if (painsPreprocessing.get(nameFilter) == false) {
				int score = 0;
				for (IAtomContainer reactant : reaction.getReactants().atomContainers()) {
					score += (int) reactant.getProperty(nameFilter);
				}
				ac.setProperty(nameFilter, score);
				result += "\t" + score;
			}
			else {
				String smarts = (String) filter[1];
		...
		for (Object[] filter : painsB) {
			Map<String, Boolean> painsPreprocessing = reactionPAINSFilter.get("painsB");
			String nameFilter = (String) filter[0];
			if (painsPreprocessing.get(nameFilter) == false) {
				int score = 0;
				for (IAtomContainer reactant : reaction.getReactants().atomContainers()) {
		...
		for (Object[] filter : painsC) {
			Map<String, Boolean> painsPreprocessing = reactionPAINSFilter.get("painsC");
			String nameFilter = (String) filter[0];
			if (painsPreprocessing.get(nameFilter) == false) {
				int score = 0;
				for (IAtomContainer reactant : reaction.getReactants().atomContainers()) {
	...
	/**
	 * Determine if the reaction center is a substrucutre of a SMARTS filter
	 * if NO, the corresponding PAINS filter will be calculated as the sum of those in reactant as no new matching substructures can be created wile generating the product
	 * If YES, the corresponding PAINS filter will used the SMARTS to recalculate it
	 * @param reactionCenter
	 */
	public Map<String, Map<String, Boolean>> processReactionCenter(String productReactionCentreSmarts) {
		Map<String, Map<String, Boolean>> result = new HashMap<String, Map<String, Boolean>>();
		Map<String, Boolean> map = new HashMap<String, Boolean>();

		//DO NOT prepare because 2 SMARTS are compared NO need to apply an aromatic model
		Pattern ptrn = SmartsPattern.create(productReactionCentreSmarts, Smarts.FLAVOR_CACTVS).setPrepare(false);
		
		for (Object[] filter : painsA) {
			String fsmarts = (String) filter[1];
			IAtomContainer target = new QueryAtomContainer(null);
			Smarts.parse(target, fsmarts);
			if (ptrn.match(target).length > 0)
				map.put((String) filter[0], true);
			else
				map.put((String) filter[0], false);
		}
		result.put("painsA", map);
		map.clear();
		...
		for (Object[] filter : painsB) {
			...
			IAtomContainer target = new QueryAtomContainer(null);
			Smarts.parse(target, fsmarts);
			//Smarts.parse(target, fsmarts, Smarts.FLAVOR_CACTVS);
			if (ptrn.match(target).length > 0)
				map.put((String) filter[0], true);
			else
				map.put((String) filter[0], false);
	...
	/**
	 * In a SMARTS convert all H into h because Aromaticity model requires implicit hydrogen count is set
	 * @param smarts
	 * @return
	 */
	private String makeSmartsCompatibleWithAromaticityModel(String smarts) {
		String newSmarts = "";
		for (int i = 0; i < smarts.length(); i++) {
			char letter = smarts.charAt(i) ;
			if (letter == 'H') {
				if (Character.isDigit(smarts.charAt(i+1)))
					newSmarts += 'h';
				else
					newSmarts += letter;
			}
	...
	//NAME SMARTS MATCHES(DEMERIT) COMMENT
	private void initPainsFilter() {
		painsA = new Object[][] {
				...
				{"anil_di_alk_C(246)","c:1:c:c(:c:c:c:1-[#8]-[#6;X4])-[#7;$([#7!H0]-[#6;X4]),$([#7](-[#6;X4])-[#6;X4])]",0,""},
