...
/**
 * SMILES {@cdk.cite WEI88,Aaq WEI89} provides a compact representation of
...
public final class SmilesGenerator {
    ...
    public static String create(IAtomContainer molecule, int flavour, int[] order) throws CDKException {
        try {
            ...
            for (int i =0; i < molecule.getAtomCount();i++) {
            	 System.out.print(" "+molecule.getAtom(i).getSymbol()+"("+molecule.getAtom(i).getProperty(ATOM_ATOM_MAPPING)+")");
            }
            System.out.println();
            System.out.println();
            for (int i =0; i < molecule.getBondCount();i++) {
 
           	 System.out.println(" "+molecule.getBond(i).getBegin().getProperty(ATOM_ATOM_MAPPING)+"("+molecule.getBond(i).getBegin().isAromatic()+")"+"--"+molecule.getBond(i).getEnd().getProperty(ATOM_ATOM_MAPPING)+"("+molecule.getBond(i).getEnd().isAromatic()+")"+"----->"+molecule.getBond(i).isAromatic());
           }
            System.out.println();
            ...
            DepictionGenerator dg = new DepictionGenerator().withSize(512, 512)
                    .withAtomColors();
            dg.depict(molecule).writeTo("~/newmol.png");

            //new DepictionGenerator().depict(molecule).writeTo("/Users/noulehoilemos2/Documents/1031/mol.png");
            ...
            DepictionGenerator dg = new DepictionGenerator().withSize(512, 512)
                    .withAtomColors();
            dg.depict(molecule).writeTo("/Users/noulehoilemos2/Documents/8000/mol.png");*/
            
           // Tools.perceiveAromaticityAndflagAtomsAndBonds(molecule);*/
    ...
    public String create(IReaction reaction, int[] ordering) throws CDKException {
       ...
       //         .withAtomColors();
       // int st = 0;
        // generate images STEFI
       /* for (IAtomContainer mol : reactants.atomContainers()) {
        	System.out.println("------");
        	try {
        		String name ="/Users/noulehoilemos2/Documents/8000/react_8000"+st+".png";
        		System.out.println("name :"+name);
				dg.depict(mol).writeTo(name);
			} catch (IOException | CDKException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        	System.out.println("reactant description : atoms ="+ mol.getAtomCount()+" bonds ="+mol.getBondCount());
        	for(int i =0; i < mol.getAtomCount();i++)
        	{
        		System.out.print(mol.getAtom(i).getSymbol()+mol.getAtom(i).getIndex() +" "+ "("+mol.getAtom(i).isAromatic()+")");
        	}
        	System.out.println();
        	
        	for(int i =0; i < mol.getBondCount();i++)
        	{
        		System.out.println(mol.getBond(i).getBegin().getIndex()+ "-"+mol.getBond(i).getEnd().getIndex()+" #"+mol.getBond(i).isAromatic());
        	}
        	System.out.println();
        	st++;
        }
        ...
        for (IAtomContainer mol : products.atomContainers()) {
        	System.out.println("------");
        	try {
        		String name ="/Users/noulehoilemos2/Documents/8000/prod_8000"+st+".png";
        		System.out.println("name :"+name);
				dg.depict(mol).writeTo(name);
			} catch (IOException | CDKException e) {
				// TODO Auto-generated catch block
        	...
        	System.out.println("product description : atoms ="+ mol.getAtomCount()+" bonds ="+mol.getBondCount());
        	for(int i =0; i < mol.getAtomCount();i++)
        	{
        		System.out.print(mol.getAtom(i).getSymbol()+mol.getAtom(i).getIndex() +" "+ "("+mol.getAtom(i).isAromatic()+")");
        	}
       ...
       /* try {
			...
			.depict(reaction).writeTo("~/reaction.pdf");
		}
		catch (Exception e) {
			e.printStackTrace();
		}*/
