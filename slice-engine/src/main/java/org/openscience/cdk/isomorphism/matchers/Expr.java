...
public final class Expr {
    ...
    private static int getPiElectronCount(IAtom atom) {
    	int pi = 0;
    	for (IBond bond : atom.bonds()) {
    ...
    private boolean matches(Type type, IAtom atom, int stereo) {
	    ...
	    case HAS_ALIPHATIC_HETERO_SUBSTITUENT:
            ...
            case HAS_R_PI_ELECTRON:
            	return getPiElectronCount(atom) > 0 ? true : false;
            case R_PI_ELECTRON_COUNT:
            	return value == getPiElectronCount(atom);
            //END modification
            case INSATURATION:
                int db = 0;
                for (IBond bond : atom.bonds())
        ...
        switch (type) {
            ...
            case ORDER:
                return bond.getOrder() != null &&
                       bond.getOrder().numeric() == value;
            case ELECTRON_COUNT:
                return bond.getElectronCount() != null &&
                       bond.getElectronCount() == value;
            case IS_AROMATIC:
                return bond.isAromatic();
            case IS_ALIPHATIC:
                return !bond.isAromatic();
    ...
    public enum Type {
        ...
        ORDER,
        /** True if the electron count {@link IBond#getElectronCount()} equals the specified
         *  value */
        ELECTRON_COUNT,
