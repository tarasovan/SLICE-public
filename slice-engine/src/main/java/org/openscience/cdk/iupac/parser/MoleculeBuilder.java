...
public class MoleculeBuilder {
    ...
    private void buildFunGroups(List<AttachedGroup> attachedGroups) {
        ...
        while (groupsIterator.hasNext()) {
            ...
            Iterator<org.antlr.v4.runtime.Token> locationsIterator = attachedGroup.getLocations().iterator();
            while (locationsIterator.hasNext()) {
                Token locationToken = (Token) locationsIterator.next();
                addFunGroup(attachedGroup.getName(), Integer.parseInt(locationToken.toString()) - 1);
            }
        }
    }
	...
	* @param mainChain The parsed prefix which depicts the chain's length.
    ...
    private void addHeads(List<AttachedGroup> attachedSubstituents) {
        ...
        while (substituentsIterator.hasNext()) {
            ...
            Iterator<org.antlr.v4.runtime.Token> locationsIterator = attachedSubstituent.getLocations().iterator();
            while (locationsIterator.hasNext()) {
                Token locationToken = (Token) locationsIterator.next();

                int joinLocation = Integer.parseInt(locationToken.toString()) - 1;
