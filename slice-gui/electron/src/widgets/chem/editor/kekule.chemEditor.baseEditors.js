...
/** @lends Kekule.Editor.BaseEditor# */
{
	...
	*/
	...
	modifyObjectsRenderOptions: function(objOrObjs, modifiedValues, is3DOption, putInOperHistory)
	{
		...
		//console.log("is3DOption:", is3DOption);
		//console.log("renderPropName : ",renderPropName);
		//console.log("getterName: ",getterName);
		//console.log("objOrObjs :",objOrObjs.symbol );
		try
		{
			var macro = new Kekule.MacroOperation();
			for (var i = 0, l = objs.length; i < l; ++i)
	...
	removeObjs: function(objs)
	{
		if (objs && objs.length)
		{
			...
			try
			{
				...
				let intersection = sel.filter(x => actualObjs.includes(x));
				if (intersection.length > 0) {
					actualObjs.push.apply(actualObjs, editor.getSelection());
