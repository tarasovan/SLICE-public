 ...
 * //BEGIN
 * @property {Int} atomMapping atom mapping type set to node
 * //END
 */
...
/** @lends Kekule.Editor.MolNodeChargeIaController# */
{
	...
	initProperties: function()
	{
		...
		///BEGIN
		this.defineProp('atomMapping', {'dataType': DataType.INT, 'serializable': false});
		//END
	...
	createOperations: function(node)
	{
		...
		//BEGIN
		var atomMapping = node.atomMappingNumber;
		if (this.atomMappingNumber && this.atomMappingNumber !== atomMapping)
		{
			modifiedData.atomMappingNumber = atomMapping;
			atomMappingModified = true;
		}
		//END
		...
		//BEGIN
		if (atomMappingModified)
		{
			var atomMappingMarker = node.fetchAtomMappingMarker(false);  // do not auto create
			if (modifiedData.atomMapping !== 0 && !atomMappingMarker)
			{
				// need add new atomMapping marker
				marker = new Kekule.ChemMarker.AtomMapping();
				marker.setValue(modifiedData.atomMapping);
				markerOperations.push(new Kekule.ChemObjOperation.Add(marker, node, null, editor));
			}
			if (modifiedData.atomMapping === 0 && atomMappingMarker)
			{
				// need remove existing atomMapping marker
				markerOperations.push(new Kekule.ChemObjOperation.Remove(atomMappingMarker, null, null, editor));
			}
		}
		//END
		...
		//BEGIN
		if (chargeModified || radicalModified || atomMappingModified)
		//END
		{
