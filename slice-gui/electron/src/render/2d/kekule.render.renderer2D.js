...
/** @lends Kekule.Render.TextBasedChemMarker2DRenderer# */
{
	...
	getRichText: function(chemObj, drawOptions)
	{
		...
		//BEGIN
		else if (obj instanceof Kekule.ChemMarker.AtomMapping)
		{
			//TODO Temp fix but need find where value is define to replicate other models
			var atomMapping = obj.value2;
			text = Kekule.Render.ChemDisplayTextUtils.getAtomMappingDisplayText(atomMapping);
			if (text) {
				if (text !== "0")
				return Kekule.Render.RichTextUtils.createSection(text,
						{'charDirection': Kekule.Render.TextDirection.LTR,'color':'#4169E1'});
			}
		}
		//END
		return null;
	...
	*/
	...
	doDrawNode: function(context, group, node, parentChemObj, options, finalTransformOptions)
	{
		...
		*/
		...
		// TODO: charge and radical drawing are now handled together, may be splitted in the future
		...
		//BEGIN
		//var needDrawAtomMapping = node.getAtomMappingNumber() > 0 ? true : false ;
		var needDrawAtomMapping = (node.getAtomMappingNumber() && !node.fetchAtomMappingMarker(false));
		//END
		...
		else
		{
			...
			// 	}
			// }
			if (needDrawCharge || needDrawRadical || needDrawAtomMapping)
			{
				/*
				var chargeOptions = this.getRenderCache(context).chargeDrawOptions;
				var localOptions = Kekule.Render.RenderOptionUtils.getChargeDrawOptions(node.getRenderOptions());
	...
	doAdjustChemPropMarkerPos: function(obj, markMargin, allowCoordBorrow, avoidDirectionAngles)
	{
		...
		try
		{
			...
			//BEGIN
			// atomMapping
			obj.fetchAtomMappingMarker(false,atomMapping);
			var atomMappingMarker = obj.getMarkerOfType(Kekule.ChemMarker.AtomMapping);
			if (atomMappingMarker) {
				var atomMapping = obj.atomMappingNumber;
				//TODO Temp fix value2 is defined for atomMapping. In the future, it needs to deepen where .value of charge and radical is define
				//but need find where value is define to replicate other model
				if (atomMapping > -1){
					atomMappingMarker.value2=atomMapping;
				}
			}

			//MOdify mark Margin to avoid overlapping with atom numbering: mappingMarkMargin has been added for this purpose
			var mappingMarkMargin = 0.35;
			this.doAutoAdjustAttachedMarkerPos(obj, atomMappingMarker, mappingMarkMargin, allowCoordBorrow, avoidDirectionAngles);
			//this.doAutoAdjustAttachedMarkerPos(obj, atomMappingMarker, markMargin, allowCoordBorrow, avoidDirectionAngles);
			//END
			// the rest
	...
	doDrawElectronStateMark: function(context, group, node, markType, partialChargeDecimalsLength, distinguishSingletAndTripletRadical, markFontFamily, markFontSize, markMargin, circleStrokeWidth, color, opacity, zoom)
	{
		...
		//BEGIN
		var atomMapping = node.getAtomMappingNumber();
		//END
		var slabel = '';
		/*
		if (!charge)
		...
		try
		{
			...
			for (var i = 0, l = mol.getNodeCount(); i < l; ++i)
			{
				...
				try
				{
					...
					//BEGIN
					if (node.getAtomMappingNumber()){
						node.fetchAtomMappingMarker(true);
					}
					//END
...
//BEGIN
Kekule.Render.Renderer2DFactory.register(Kekule.ChemMarker.AtomMapping, Kekule.Render.TextBasedChemMarker2DRenderer);
//END
