...
(function()
{
	...
	/** @lends Kekule.ChemStructureNode# */
	{
		 ...
		 * @param {Kekule.Render.DisplayLabelConfigs} displayLabelConfigs
		 */
		 getDisplayRichText: function(hydrogenDisplayLevel, showCharge, showAtomMapping, displayLabelConfigs, partialChargeDecimalsLength, chargeMarkType, distinguishSingletAndTripletRadical)
		 {
			 var R = Kekule.Render;
			 if (Kekule.ObjUtils.isUnset(showCharge))
				 showCharge = true;
		...
		appendElectronStateDisplayText: function(coreItem, partialChargeDecimalsLength, chargeMarkType, distinguishSingletAndTripletRadical)
		{
			...
			var charge = this.getCharge();
			var radical = this.getRadical();
			var atomMapping = this.getAtomMappingNumber();
			var section = R.ChemDisplayTextUtils.createElectronStateDisplayTextSection(charge, radical, atomMapping, partialChargeDecimalsLength, chargeMarkType, distinguishSingletAndTripletRadical);
			if (section)
			{
				//richText = R.RichTextUtils.append(richText, section);
				var group = R.RichTextUtils.createGroup();
	...
	/** @lends Kekule.VariableAtom# */
	{
		...
		getCoreDisplayRichTextItem: function(hydrogenDisplayLevel, showCharge, displayLabelConfigs, partialChargeDecimalsLength)
		{
			...
			if (this.isListEmpty())
			{
				...
				result = R.RichTextUtils.strToRichText(NL.VARIABLE_ATOM);
				//END
				result.charDirection = Kekule.Render.TextDirection.LTR;
				return result;
			}
			...
			if (isDisallowed)
				...
				result = R.RichTextUtils.appendText(result, NL.ISO_LIST_DISALLOW_PREFIX);
				//END
			//BEGIN Correct Bug
			//result = R.RichTextUtils.appendText(result, D.getIsoListLeadingBracket());
			result = R.RichTextUtils.appendText(result, NL.ISO_LIST_LEADING_BRACKET);
			//END
				...
				for (var i = 0, l = isotopeIds.length; i < l; ++i)
				{
					...
					if (id)
					{
						if (i != 0)  // not leading isotope, append delimiter
							...
							result = R.RichTextUtils.appendText(result, NL.ISO_LIST_DELIMITER );
							//END
			...
			result = R.RichTextUtils.appendText(result, NL.ISO_LIST_TAILING_BRACKET);
			//END
			return result;
		}
	});
	...
	/** @lends Kekule.MolecularFormula# */
	{
		getDisplayRichText: function(showCharge, displayLabelConfigs, partialChargeDecimalsLength, chargeMarkType, distinguishSingletAndTripletRadical)
		{
			...
			//END
			//BEGIN
			return R.ChemDisplayTextUtils.formulaToRichText(this, showCharge, null, null, partialChargeDecimalsLength, displayLabelConfigs, chargeMarkType, distinguishSingletAndTripletRadical);
			//END
		},
		/**
