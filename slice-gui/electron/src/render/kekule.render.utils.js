...
Kekule.Render.ChemDisplayTextUtils = {
	...
	//BEGIN
	/**
	 * Returns text to represent atom Mapping
	 * @param {Number} atomMapping
	 * @returns {String}
	 */
	 getAtomMappingDisplayText: function(atomMapping)
	 {
		 if (atomMapping)
		 return atomMapping.toString();
		 else
		  return '';
	 },
	 //END
	 ...
	 * @param {Number} charge
	 * @param {Int} radical
	 * @param {Int} atomMapping
	 * @param {Int} partialChargeDecimalsLength
	 * @param {Int} chargeMarkType
	 * @returns {Object}
	 */
	 createElectronStateDisplayTextSection: function(charge, radical, atomMapping, partialChargeDecimalsLength, chargeMarkType, useAlterTripletRadicalMark)
	 {
		 var result = null;
		 var slabel = '';
		 /*
		 ...
		 */
		 ...
		 if (atomMapping)
		 {
			 slabel += Kekule.Render.ChemDisplayTextUtils.getAtomMappingDisplayText(atomMapping) || '';
		 }
	 ...
	 * @returns {Object}
	 */
	 formulaToRichText: function(formula, showCharge, showRadical, showAtomMapping, partialChargeDecimalsLength, displayConfigs, chargeMarkType, distinguishSingletAndTripletRadical)
	 {
		 //var result = Kekule.Render.RichTextUtils.create();
		 var result = Kekule.Render.ChemDisplayTextUtils._convFormulaToRichTextGroup(formula, false, showCharge, showRadical, showAtomMapping, partialChargeDecimalsLength, displayConfigs, chargeMarkType, distinguishSingletAndTripletRadical);
		 return result;
	 },
 
	 /** @private */
	 _convFormulaToRichTextGroup: function(formula, showBracket, showCharge, showRadical, showAtomMapping, partialChargeDecimalsLength, displayConfigs, chargeMarkType, distinguishSingletAndTripletRadical)
	 {
		 ...
		 for (var i = 0, l = sections.length; i < l; ++i)
		 {
			 ...
			 if (obj instanceof Kekule.MolecularFormula)  // a sub-formula
			 {
				 ...
				 subgroup = Kekule.Render.ChemDisplayTextUtils._convFormulaToRichTextGroup(obj, true, false, false, false, partialChargeDecimalsLength, displayConfigs, chargeMarkType); // do not show charge right after, we will add it later
			 }
			 ...
			 if (subgroup)
			 {
				 ...
				 {
					 var chargeSection = Kekule.Render.ChemDisplayTextUtils.createElectronStateDisplayTextSection(charge, null, null, partialChargeDecimalsLength, chargeMarkType, distinguishSingletAndTripletRadical);
					 if (chargeSection)
					 {
		 ...
		 if (showCharge || showRadical || showAtomMapping)
		 {
			 var charge = formula.getCharge();
			 var radical = formula.getRadical();
			 var atomMapping = formula.getAtomMappingNumber();
			 var chargeSection = Kekule.Render.ChemDisplayTextUtils.createElectronStateDisplayTextSection(charge, radical, atomMapping, partialChargeDecimalsLength, chargeMarkType);
			 if (chargeSection)
			 {
...
Kekule.Render.RenderOptionUtils = {
	...
	getOptionDefinitions: function()
	{
		return [
			...
			//BEGIN
			{'name': 'showAtomMapping', dataType: DataType.BOOL, 'targetClass': Kekule.ChemStructureNode},
			{'name': 'atomMappingMarkType', dataType: DataType.INT, 'enumSource':  Kekule.Render.AtomMappingMarkRenderType, 'targetClass': Kekule.ChemStructureNode},
			{'name': 'atomMappingMarkFontSize', dataType: DataType.FLOAT, 'targetClass': Kekule.ChemStructureNode},
			{'name': 'atomMappingMarkMargin', dataType: DataType.FLOAT, 'targetClass': Kekule.ChemStructureNode},
			//END
	 ...
	 * @returns {Bool}
	 */
	 getShowAtomMapping: function(renderOptions)
	 {
		 return renderOptions? renderOptions.showAtomMapping: null;
	 },
	 getAtomMappingDrawOptions: function(renderOptions)
	 {
		 if (!renderOptions)
			 return null;
		 else
		 {
			 var props = ['showAtomMapping', 'atomMappingMarkType', ' atomMappingMarkFontSize',
				 'atomMappingMarkMargin',
				 'color', 'opacity'];
			 var result = Object.copyValues({}, renderOptions, props);
			 return result;
		 }
	...
	*/
	...
	convert2DConfigsToPlainHash: function(render2DConfigs)
	{
		...
		*/
		...
		//BEGIN
		OU.replacePropName(h, 'defAtomMappingMarkType', 'atomMappingMarkType');
		//END
		result = Object.extend(result, h);
